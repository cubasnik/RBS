...
for d in DIRS:
    ...
    for fname in os.listdir(folder):
        ...
        # Fix encoding_constraints struct initializer: replace conditional
        # positional initializers with designated .per_constraints/.general_constraints.
        # Without this, when ASN_DISABLE_OER_SUPPORT is defined, the PER
        # constraint pointer shifts into the oer_constraints slot (wrong).
        # Case 1: PER constraint is a non-null pointer (&asn_PER_..._constr_N)
        ENC_CONSTR_PATTERN = re.compile(
            r'\{\s*\n'
            r'(?:#if[^\n]*\n\s*0,\n#endif[^\n]*\n)?'   # optional OER 0
            r'(?:#if[^\n]*\n\s*(&\w+),\n#endif[^\n]*\n)'  # PER ptr (captured)
            r'(?:#if[^\n]*\n\s*0,\n#endif[^\n]*\n)?'   # optional JER 0
            r'\s*(\w+)\s*\n'                            # general_constraints (captured)
            r'\s*\}',
            re.MULTILINE
        )
        def fix_enc_constr(m):
            per_ptr = m.group(1)
            general = m.group(2)
            return f'{{ .per_constraints = {per_ptr}, .general_constraints = {general} }}'
        new_text = ENC_CONSTR_PATTERN.sub(fix_enc_constr, text)
        if new_text != text:
            text = new_text

        # Case 2: PER constraint is 0 but general_constraints is non-null (function ptr).
        # e.g. OPEN TYPE entries: { 0, OPEN_TYPE_constraint } maps poorly unless designated.
        ENC_CONSTR_ZERO_PER = re.compile(
            r'\{\s*\n'
            r'(?:#if[^\n]*\n\s*0,\n#endif[^\n]*\n)?'   # optional OER 0
            r'(?:#if[^\n]*\n\s*0,\n#endif[^\n]*\n)'    # PER = 0 (required for this pattern)
            r'(?:#if[^\n]*\n\s*0,\n#endif[^\n]*\n)?'   # optional JER 0
            r'\s*(\w+)\s*\n'                            # general_constraints (captured, non-zero)
            r'\s*\}',
            re.MULTILINE
        )
        def fix_enc_constr_zero_per(m):
            general = m.group(1)
            # Only patch if the general_constraints is not '0' (avoid turning { 0 } into designated)
            if general == '0':
                return m.group(0)
            return f'{{ .general_constraints = {general} }}'
        new_text = ENC_CONSTR_ZERO_PER.sub(fix_enc_constr_zero_per, text)
        if new_text != text:
            text = new_text
        ...
        # INTEGER.c: fix unsigned integer range encoding for types with ub=ULONG_MAX
        # (e.g. MME-UE-S1AP-ID: 0..4294967295 overflows to ub=-1 in signed long).
        # The generated per_long_range_rebase call asserts lb <= ub which fails.
        if fname == 'INTEGER.c':
            OLD_RANGE = (
